...
| Document intake | Phase 1 | Built (`documentExtraction.ts`, `POST /api/hangar/extract-document`) — the file is uploaded, its text extracted server-side, and **the file itself is never persisted**: no storage bucket, no `Hangar_mission_documents` table (the spec's Section 10 design). Only the extracted text travels on, folded into `rawTextCombined` exactly like typed text, so it's the caller's own content — no "untrusted reference data" framing is needed (unlike market-data links, which fetch someone else's page). Type is checked by content (magic bytes / valid-UTF-8-with-low-control-character-ratio for plain text), never by the browser-supplied MIME type or file extension, both of which can be wrong. Limits: 4 MB upload (Vercel's Node functions hard-cap the request body around 4.5 MB regardless of any limit set here), 20,000 characters kept, 15s extraction timeout. A file that can't be read never fails the mission — nothing is added from that source. Uses `pdf-parse` (PDF) and `mammoth` (.docx) |
