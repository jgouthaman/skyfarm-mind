...
### 3.g Re-verify `source_mission_id` transitively — RESOLVED
...
Two options were on the table, not a recommendation for one over the other — this needed a decision when Bay 03 was actually scoped, not a default assumed here:
...
If Bay 03 never reads mission-level data at all (only ever reads the concept's own already-persisted `trade_off_notes`/`ranked_concepts`), this gap wouldn't reach Bay 03 and neither option would have been required yet — but that would have needed to be a deliberate scoping decision, not an assumption made by omission.

**Resolution (commit `28b8c8f`):** Option 1 was chosen. `assertMissionOwnership(sourceMissionId, userId)` was added to `conceptAgentPipeline.ts` and is called at the top of `runConceptIdeationStage`, before any use of `sourceMissionId` and before the `Hangar_concepts` row is created. It verifies, in order: the mission row exists ("Source mission not found"), it belongs to the requesting user ("Source mission does not belong to this user"), and its status is `'finalized'` ("Source mission is not finalized") — throwing on the first failing check. This closes the gap at its source, per Option 1's own reasoning above: every future consumer of concepts benefits from this check, not just Bay 03, and Bay 03 no longer needs to independently re-verify this chain itself (Option 2) regardless of whether it ends up reading mission-level data directly.
