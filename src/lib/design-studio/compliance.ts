// DGCA + NPNT compliance helpers for Indian airspace (CAR 2021 / Drone Rules 2021).
// Indicative only — operators must verify with DGCA DigitalSky before any flight.

import type { DroneProject } from "./types";

export type DgcaCategory = "Nano" | "Micro" | "Small" | "Medium" | "Large";

export interface ComplianceReport {
  allUpWeightKg: number;
  category: DgcaCategory;
  weightBand: string;
  npntRequired: boolean;
  uinRequired: boolean;
  rpcRequired: boolean;          // Remote Pilot Certificate
  typeCertRequired: boolean;
  insuranceRequired: boolean;
  thirdPartyInsuranceRequired: boolean;
  maxAltitudeFtAgl: number;
  requiresDigitalSkyPermission: boolean;
  requiresLocalPolicePermission: boolean;
  requiresATCApproval: boolean;
...
export interface StateRule {
  ...
  zoneNote: string;
  authority: string;
  permissionWindowDays: number;     // typical lead time
  knownRedZones: string[];
  recommendation: string;
}

// Indian states — practical operating notes (indicative; verify with state authorities)
export const INDIAN_STATES: StateRule[] = [
...
export function classifyDrone(allUpWeightKg: number): { category: DgcaCategory; band: string } {
...
export function buildComplianceReport(project: DroneProject, auwOverride?: number): ComplianceReport {
  ...
  return {
    allUpWeightKg: allUp,
    category, weightBand: band,
    npntRequired, uinRequired, rpcRequired, typeCertRequired,
    insuranceRequired, thirdPartyInsuranceRequired: thirdPartyInsurance,
    maxAltitudeFtAgl,
